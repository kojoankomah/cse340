...
function inventoryRules() {
  return [
    body('classification_id').trim().notEmpty().isInt().withMessage('Please choose a classification.'),
    body('inv_make').trim().escape().notEmpty().isLength({ min: 2 }).withMessage('Please provide the make (at least 2 characters).'),
    ...
    body('inv_price').trim().notEmpty().isFloat({ min: 0 }).withMessage('Please provide a valid price.'),
    body('inv_miles').trim().notEmpty().isInt({ min: 0 }).withMessage('Please provide mileage (0 or more).'),
    body('inv_color').trim().escape().notEmpty().withMessage('Please provide a color.'),
    body('inv_description').trim().escape().notEmpty().isLength({ min: 10 }).withMessage('Please provide a description (at least 10 characters).'),
...
async function checkInventoryData(req, res, next) {
  ...
  if (!errors.isEmpty()) {
    ...
    // Pass errors as array for easy looping
